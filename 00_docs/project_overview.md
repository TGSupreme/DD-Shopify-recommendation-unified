...
*   **Semantic Search:** Allows customers to find products based on concepts and descriptions rather than exact keyword matches. Enhanced with **Two-Stage Retrieval (Neural Reranking)** for industry-leading precision.
...
### 3. Two-Stage Retrieval: High Precision Funnel
To overcome the "blurriness" of traditional vector searches, the engine implements a high-precision funnel:

1.  **Stage 1: Fast Retrieval (The Net):** Qdrant retrieves the Top 50 candidates in <20ms using Jina AI embeddings.
2.  **Stage 2: Neural Reranking (The Judge):** Jina Reranker v2 performs a deep linguistic comparison between the query and the top candidates to ensure the #1 result is the most relevant.
3.  **Final Step: Diversity Logic (MMR):** The top reranked items are filtered for variety to avoid showing repetitive products.

---

### 4. How It Works: The "Tri-tier" Schema
